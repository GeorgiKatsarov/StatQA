...
async function inspectPage(page: Page, url: string, origin: string): Promise<SitePageEvidence> {
  ...
  const snapshot = await page.evaluate(() => {
    ...
    const links = Array.from(document.querySelectorAll("a[href]"))
      .filter(isVisible)
      .map((element) => ({ text: elementText(element).slice(0, 120), href: (element as HTMLAnchorElement).href }))
    ...
    const inputs = Array.from(document.querySelectorAll("input, textarea, select"))
      ...
      .map((element) => ({
        ...
        label: cleanText(element.getAttribute("aria-label")) || undefined,
    ...
    return { title: cleanText(document.title), headings, buttons, links, inputs, forms, textSnippets };
  });
  ...
  return {
    ...
    links: snapshot.links.map((link) => ({ ...link, internal: safeSameOrigin(link.href, origin) })),
    ...
    const result = await callGroqJson([
      {
        role: "system",
        content:
          "You are a Playwright architect. Create automation blueprints only from observed evidence. Do not invent selectors, routes, credentials, clicks, or data. Return valid JSON only."
      },
      {
        role: "user",
